...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (body.length > MAX_INVOICES) {
      return NextResponse.json(
        { error: `Too many invoices. Maximum is ${MAX_INVOICES}` },
        { status: 400 }
      );
    }
    ...
    if (body.length === 0) {
      return NextResponse.json(
        { error: 'Empty array. At least one invoice is required.' },
        { status: 400 }
      );
    }
    ...
    for (let i = 0; i < body.length; i++) {
      ...
      } else if (typeof invoice.shipment.trackingNumber !== 'string' || invoice.shipment.trackingNumber.length > 100) {
        errors.push(`Faktura ${i + 1}: neplatný tracking number (maximální délka 100 znaků)`);
      }
      ...
      if (!invoice.shipment.company) {
        ...
        } else if (typeof invoice.shipment.company.name !== 'string' || invoice.shipment.company.name.length > 255) {
          errors.push(`Faktura ${i + 1}: název společnosti je příliš dlouhý (maximálně 255 znaků)`);
        }
      }
      ...
      } else if (invoice.invoicedWeight > 1000000) {
        errors.push(`Faktura ${i + 1}: váha je příliš velká (maximálně 1,000,000)`);
      }
      ...
      } else if (invoice.invoicedPrice > 1000000000) {
        errors.push(`Faktura ${i + 1}: cena je příliš velká (maximálně 1,000,000,000)`);
      }
